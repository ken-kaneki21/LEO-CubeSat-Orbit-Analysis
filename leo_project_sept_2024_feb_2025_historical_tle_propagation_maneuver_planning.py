...
NORAD_IDS = [99999]  # e.g., [25544] for ISS (for a quick test)
...
def main():
    ...
    for row in epochs:
        ...
        plot_groundtrack(
            lats, lons,
            title=f"Ground Track (24h) from TLE epoch {row.epoch_dt.isoformat()} UTC
NORAD {row.norad}",
            outfile=gt_file,
            gs_lat=GROUND_STATION_LAT, gs_lon=GROUND_STATION_LON)
        plot_altitude(
            hh, alts,
            title=f"Altitude vs Time (24h) from TLE epoch {row.epoch_dt.isoformat()} UTC
NORAD {row.norad}",
            outfile=at_file)
